...
const allowedOrigins = [frontendLocalUrl, frontendCodespaceUrl, `http://localhost:${PORT}`].filter((origin): origin is string => Boolean(origin));
...
app.get('/api/users', async (req, res) => {
  const users = (await User.find({}).exec()) as Record<string, unknown>[];
...
app.get('/api/teams', async (req, res) => {
  const teams = (await Team.find({}).exec()) as Record<string, unknown>[];
...
app.get('/api/activities', async (req, res) => {
  const activities = (await Activity.find({}).exec()) as Record<string, unknown>[];
...
app.get('/api/leaderboard', async (req, res) => {
  const leaderboard = (await Leaderboard.find({}).sort({ rank: 1 }).exec()) as Record<string, unknown>[];
...
app.get('/api/workouts', async (req, res) => {
  const workouts = (await Workout.find({}).exec()) as Record<string, unknown>[];
